...
//		int offset=0;
		...
		//Vector v = edges[1];
			...
			Vector cOrth = center.getOrthogonalComponentTo(v);
			for (Vector b : points) {// for each point of this rectangle
				Vector parallel = b.getOrthogonalComponentTo(v);
				float l2 = parallel.length();// l is the projection of the point onto v
				//parallel.draw(debugger);
				//debugger.point(500, 500);
				//if(parallel.x<-0.0000001&&parallel.y>0.0000001)
					//l2*=-1;
				//parallel.addN(new Vector(500,500)).draw(debugger);
				//center.getOrthogonalComponentTo(v).addN(new Vector(500,500)).draw(debugger);
				if(cOrth.dot(parallel)<0)
					l2*=-1;
				if (l2 > maxThis)
					maxThis = l2;
				if (l2 < minThis)
			...
			float l3 = cOrth.length();
//			debugger.strokeWeight(5);
//			debugger.stroke(255,0,0);
//			debugger.point(maxThis, 200+offset);
//			debugger.stroke(0,255,0);
//			debugger.point(minThis,200+offset);
